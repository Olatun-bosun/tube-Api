...
//            _ffmpegPath = configuration["FFmpegPath"]; // Assumes yt-dlp is in PATH
...
//                if (!result.Success)
//                {
//                    return BadRequest($"Failed to get video info: {result.Error}");
//                }
...
//            catch (Exception ex)
//            {
//                _logger.LogError(ex, "Error downloading audio: {Url}", request.VideoUrl);
//                return StatusCode(500, $"Internal server error: {ex.Message}");
//            }
//        }
...
//            {
//                "best" => "best[ext=mp4]/best", // Single file, best quality
//                "best-merge" => "bestvideo[ext=mp4]+bestaudio[ext=m4a]/bestvideo+bestaudio", // Requires FFmpeg
...
//                _ => "best[ext=mp4]/best" // Default to best single-file quality
//            };
//        }
...
//            return formats.Take(10).ToList(); // Limit to avoid huge responses
//        }
//    }
...
//        public string? Quality { get; set; } = "best"; // best, 720p, 480p, 360p, worst
//    }
...
//        // APPROACH 2: Direct streaming download
...
//                if (!result.Success)
//                {
//                    return BadRequest($"Failed to get direct URL: {result.Error}");
//                }
...
//                // Get filename - using the properly typed method now
//                var videoInfo = await GetVideoInfoInternal(request.VideoUrl);
//                // Fix for CS1061: Cast the 'videoInfo' object to the expected type 'VideoInfo' before accessing the 'Title' property.
//                var fileName = $"{(videoInfo as VideoInfo)?.Title ?? "video"}.mp4";

//                // Return redirect to direct URL (browser will handle the download with progress)
//                return Redirect(directUrl);

//                // Alternative: Stream through our server (more control but uses bandwidth)
//                // return await StreamFromUrlAsync(directUrl, fileName);
//            }
//            catch (Exception ex)
//            {
